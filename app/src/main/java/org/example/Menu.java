...
public class Menu{ 
  ...
  public void playPvAI(){
    TTTAISystem system = new TTTAISystem();
    boolean playAgain = true;
    boolean firstTime = true;

    boolean playersTurn = true; // true = X, ai = O
    while(playAgain){
      ...
      while (board.checkWin() == 1 && turn < 10){
        ...
        System.out.println("It is " + (playersTurn ? "X" : "O") + "'s turn.");

        int userInput;
        if (playersTurn){
          userInput = getIntInput("Enter the number of the square you would like to place your piece: ");
        } else {
          userInput = system.getBestMove(board)+1;
        }
